...
class TCPHandler(socketserver.BaseRequestHandler):
    ...
    def handleBlockstatsReq(self, peer):
        with self.chain_lock:
            ...
            difficulty = str(
                self.active_chain.chain[-1].nonce
                / 1000
                / (
    ...
    def handleBlockSyncReq(self, blockid: str, peer: Peer):
        ...
        if (peer not in self.peerManager.getAllPeers()) and not (
    ...
    def handleBlockSyncGet(self, blocks: Iterable[Block], peer: Peer):
        ...
        new_blocks = [
            block
            for block in blocks
            if not Block.locate_block(block.id, self.active_chain, self.side_branches)[
                0
            ]
        ]
        logger.info(
        ...
        with self.chain_lock:
            ...
            for block in new_blocks:
                if Block.locate_block(block.id, self.active_chain, self.side_branches)[
                    0
                ]:
                    new_blocks.pop(0)
                else:
            ...
            chain_idx = TCPHandler.check_block_place(
                new_blocks[0],
                self.active_chain,
                self.utxo_set,
                self.mempool,
            ...
            for block in new_blocks:
                if not TCPHandler.do_connect_block_and_after(
                    block,
                    chain_idx,
                    self.active_chain,
                    self.side_branches,
        ...
        if chain_idx == Params.ACTIVE_CHAIN_IDX:
            if (
                len(self.active_chain.chain) % Params.SAVE_PER_SIZE == 0
                or len(self.active_chain.chain) <= 5
            ):
        ...
        try:
            ...
            if message:
                ...
                message = Message(
                    message.action, message.data, Params.PORT_CURRENT, peer
                )
                ret = Utils.send_to_peer(
                    message, Peer("127.0.0.1", Params.PORT_CURRENT), itself=True
                )
                if ret != 0:
                    logger.info(
            ...
            if mempool_ret:
                ...
                if len(peers) > 0:
                    for _peer in random.sample(peers, min(len(peers), 5)):
                        if _peer != peer:
                            ret = Utils.send_to_peer(
                                Message(Actions.TxRev, txn, Params.PORT_CURRENT), _peer
                            )
                            if ret == 1:
                                if _peer in peers:
                                    try:
        ...
        if not isinstance(block, Block):
            ...
            with self.chain_lock:
                ...
                if chain_idx is not None and chain_idx >= 0:
                    ret_outside_lock = TCPHandler.do_connect_block_and_after(
                        block,
                        chain_idx,
                        self.active_chain,
                        self.side_branches,
                    ...
                    if not Block.locate_block(
                        block.id, self.active_chain, self.side_branches
                    )[0]:
                        return
            ...
            if chain_idx is not None and chain_idx >= 0:
                ...
                if len(peers) > 0:
                    for _peer in random.sample(peers, min(len(peers), 5)):
                        if _peer != peer:
                            ret = Utils.send_to_peer(
                                Message(Actions.BlockRev, block, Params.PORT_CURRENT),
                                _peer,
                            )
                            if ret == 1:
                                if _peer in peers:
                ...
                try:
                    ...
                    message = (
                        Utils.deserialize(data.decode(), self.gs) if data else None
                    )
                    if message:
                        blocks = message.data
    ...
    def handlePeerExtendGet(self, peer_samples: Iterable[Peer], peer: Peer):
        ...
        for peer_sample in peer_samples:
            ...
            if (
                peer_sample == Peer("127.0.0.1", Params.PORT_CURRENT)
                or peer_sample == Peer("localhost", Params.PORT_CURRENT)
                or peer_sample.ip == "0.0.0.0"
                or peer_sample == Peer(Params.PUBLIC_IP, Params.PORT_CURRENT)
            ):
                continue
    ...
    def do_connect_block_and_after(
        ...
        if int(chain_idx) == int(Params.ACTIVE_CHAIN_IDX):
            if block.block_subsidy_fees != Block.get_block_subsidy(
                ...
                pass
            connect_block_success = active_chain.connect_block(
                block, active_chain, side_branches, mempool, utxo_set, mine_interrupt
            )
        else:
            connect_block_success = side_branches[chain_idx - 1].connect_block(
                block, active_chain, side_branches, mempool, utxo_set, mine_interrupt
            )
        ...
        if connect_block_success is not False:
                ...
                for branch_chain in side_branches:
                    ...
                    for block in branch_chain.chain[::-1]:
                        if not Block.locate_block(block.id, active_chain)[0]:
                            if not Block.locate_block(
                                block.prev_block_hash, active_chain
                            )[0]:
                            ...
                            break
                    if (
                        fork_height_from_end >= branch_chain.height
                        and branch_chain.height != 0
                    ):
                        branch_chain.chain = []
                        logger.info(
            ...
            for branch_chain in side_branches:
                ...
                if (
                    active_chain.height - branch_height_real
                    > Params.MAXIMUM_ALLOWABLE_HEIGHT_DIFF
                ):
                    side_branches_to_discard.append(branch_chain)
    ...
    def check_block_place(
        ...
        try:
            chain_idx = block.validate_block(
                active_chain, utxo_set, mempool, side_branches
            )
        ...
        if chain_idx != Params.ACTIVE_CHAIN_IDX and len(side_branches) < chain_idx:
            ...
            if prev_block_chain_idx != Params.ACTIVE_CHAIN_IDX:  # branch of a branch
                ...
                branch_fork_height = Block.locate_block(
                    block.prev_block_hash, side_branches[prev_block_chain_idx - 1]
                )[1]
                side_branches[chain_idx - 1].chain = list(
                    side_branches[prev_block_chain_idx - 1].chain[:branch_fork_height]
                )
    ...
    def check_blocks_headers(cls, new_blocks: Iterable[Block]) -> bool:
        for idx in range(len(new_blocks) - 1):
            ...
            if MerkleNode.get_merkle_root_of_txns(block.txns).val != block.merkle_hash:
                logger.info(
                    f"[p2p] check of block headers is a failure for not wrong merkle hash given"
                )
                return False
            elif block.id != new_blocks[idx + 1].prev_block_hash:
                logger.info(
                    f"[p2p] check of block headers is a failure for not consistent block hash: block.id is {block.id}, and prev_block hash is {new_blocks[idx+1].prev_block_hash}"
                )
                return False
            else:
                return True
    ...
    def sendPeerExtend(self):
        ...
        if len(peers) > 0:
            for _peer in peers:
                ...
                ret = Utils.send_to_peer(
                    Message(Actions.PeerExtend, peer_samples, Params.PORT_CURRENT),
                    _peer,
                )
                if ret == 1:
                    if _peer in peers:
