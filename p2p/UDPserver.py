...
class ThreadedUDPServer(socketserver.ThreadingMixIn, socketserver.UDPServer):
    def __init__(self, ip_port, udp_handler_class, active_chain: BlockChain, side_branches: Iterable[BlockChain], \
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        # self.server is an instance of the ThreadedUDPServer
        ...
        if not isinstance(message, Message):
            logger.exception(f'[p2p] Not a Message from peer {self.client_address}')
            return
        else:
            peer = Peer(str(self.client_address[0]), int(message.port))
        ...
        with self.chain_lock:
            for block in new_blocks:
                ...
                if chain_idx is not None and chain_idx >= 0:
                    if not UDPHandler.do_connect_block_and_after(block, chain_idx, self.active_chain, self.side_branches, \
    ...
    def handleBlockRev(self, block: Block, peer: Peer):
        if isinstance(block, Block):
            ...
            with self.chain_lock:
                chain_idx  = UDPHandler.check_block_place(block, self.active_chain, self.utxo_set, self.mempool, \
                                                          self.side_branches)
                if chain_idx is not None and chain_idx >= 0:
                    if not UDPHandler.do_connect_block_and_after(block, chain_idx, self.active_chain, self.side_branches, \
                                                       self.mempool, self.utxo_set, self.mine_interrupt, self.peers):
