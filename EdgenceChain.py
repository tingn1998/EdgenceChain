...
class EdgenceChain(object):
    ...
    def assemble_and_solve_block(self, txns=None) -> Block:
        ...
        with self.chain_lock:
            ...
            prev_block_hash = (
                self.active_chain.chain[-1].id if self.active_chain.chain else None
            )
            ...
            block = Block(
                ...
                bits=Block.get_next_work_required(
                    prev_block_hash, self.active_chain, self.side_branches
                ),
                nonce=0,
                # None is a placeholder for coinbase txns to be created later.
            ...
            # If txns = [None], choose txns from mempool.
            if not block.txns[1:]:
            ...
            # Create coinbase transaction in this block.
    ...
    def start(self):
        ...
        def mine_forever():
            ...
            def broadcast_new_mined_block(block_to_broadcast):
                ...
                for _peer in peers:
                    ret = Utils.send_to_peer(
                        Message(
                            Actions.BlockRev, block_to_broadcast, Params.PORT_CURRENT
                        ),
                        _peer,
                    )
                    if ret == 1:
        ...
        def initiative_sync():
            ...
            def work_to_do():
                logger.info(f"begin to work in a new thread of initiative_sync")
                try:
                    with self.peers_lock:
                    ...
                    with self.chain_lock:
                        for block in self.orphan_blocks:
                            if (
                                ...
                                * Params.TIME_BETWEEN_BLOCKS_IN_SECS_TARGET
                            ):
                ...
                if len(getpeers) > 0:
                    ...
                    try:
                        message = Message(
                            Actions.TopBlockReq, None, Params.PORT_CURRENT
                        )
                        ...
                        if message:
                            ...
                            ret = Utils.send_to_peer(
                                message,
                                Peer("127.0.0.1", Params.PORT_CURRENT),
                                itself=True,
                            )
                            if ret != 0:
                                logger.info(
        ...
        while new_height > old_height:
            ...
            logger.info(
                f"{int(new_height-old_height)} more blocks got this time, waiting for blocks syncing ..."
            )
        self.ibd_done.set()
